...
export const Sidebar = () => {
    ...
    return (
        <section className={`${isSidebarOpen?"w-3/7":" w-26"}  h-full border-default rounded-2xl border-1 transition-all duration-500 flex flex-col p-6 gap-2 background-default`} 
                id="sidebar"
                onMouseEnter={() => logic(true)}
                onMouseLeave={() => logic(false)}
        >
            <UserSection sidebarMode={sidebarMode} setSidebarMode={setSidebarMode}/>
            {!isInRoom && <CreateRoomSection sidebarMode={sidebarMode} setSidebarMode={setSidebarMode}/>}
            {isInRoom && <SearchSection sidebarMode={sidebarMode} setSidebarMode={setSidebarMode}/>}
            <PlaylistSection sidebarMode={sidebarMode} setSidebarMode={setSidebarMode}/>
        </section>
    )
}
    ...
    return (
        <div className="h-fit min-h-12 border-b-1 flex flex-col">
            <span onClick={()=>setSidebarMode((sidebarMode==="user")?"default":"user")}
                  className="w-full m-auto cursor-pointer h-12 leading-[3rem] text-xl"
            >
            사용자
            </span>
            <div className={`${sidebarMode === "user"?"h-40":"h-0"} overflow-hidden transition-h duration-300`}>
...
const CreateRoomSection = (props:SidebarSectionProps) => {
    ...
    return (
        <div className="h-fit min-h-12 border-b-1 flex flex-col">
            ...
            <div className={`${sidebarMode === "create"?"h-30":"h-0"} overflow-hidden transition-h duration-300`}>
                <input type="text" placeholder="방 이름" className='w-full h-10 border-1 border-default rounded mb-2' />
                <button className='w-full h-10 border-1 border-default rounded'
                        onClick={()=>sendSocket()}
                >
...
const SearchSection = (props:SidebarSectionProps) => {
    ...
    return (
        <div className="h-fit min-h-12 border-b-1 flex flex-col m-0 p-0">
            ...
            <div className={`${sidebarMode === "search"?"h-[calc(100vh-20rem)]":"h-0"} overflow-y-scroll transition-h duration-300 max-h-full scrollbar-hide`}>
                <Search />
            </div>
        </div>
    )
...
const PlaylistSection = (props:SidebarSectionProps) => {
    ...
    return (
        <div className="h-fit min-h-12 border-b-1 flex flex-col m-0 p-0">
            ...
            <div className={`${sidebarMode === "playlist"?"h-[calc(100vh-20rem)]":"h-0"} overflow-scroll transition-h duration-300 max-h-full`}>
                <Playlist />
            </div>
        </div>
    )
